...
<TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
  ...
  <TD style="PADDING-LEFT: 20px; FONT-WEIGHT: bold; COLOR: #ffffff" 
    align=left background=images/title_bg2.jpg>当前位置: 在线解压ZIP文件程序</TD></TR>
</TABLE>
...
<table width=95% align=center border=0 cellpadding=0 cellspacing=0><tr>
  <td><table width=100% border=0 cellpadding=0 cellspacing=0>
    <tr>
      <td width="11%" height="35" align="center"><strong>选择ZIP文件</strong></td>
...
?>
    ...
    <tr>
      <td width="11%" height="35" align="center"><strong>或上传文件</strong></td>
      <td width="89%"><input name="upfile" type="file" id="upfile" size="20"></td>
    </tr>
	<tr>
      <td height="35" align="center"><strong>解压到目录</strong></td>
      <td><input name="todir" type="text" id="todir" value="../template" size="15"> (留空为本目录,目录必须有写入权限)</td>
    </tr>	
    <tr>
      <td height="35" align="center"><input name="myaction" type="hidden" id="myaction" value="dounzip"></td>
      <td><input type="submit" name="Submit" value=" 解 压 "></td>
    </tr>
    <tr>
      <td height="30" colspan="2" style="color:#FF9900">
      使用方法:把zip文件通过FTP上传到本文件相同的目录下,选择zip文件;或直接点击&ldquo;浏览...&rdquo;上传zip文件。<br>
      解压的结果保留原来的目录结构。</td>
    </tr>
  </table>
</td></tr></table>
 ...
 function ExtractFile($header,$to,$zip){
	...
	for($i=0;$i<count($pth)-1;$i++){
		...
		if((!is_dir($mydir) && @mkdir($mydir,0777)) || (($mydir==$to.$header['filename'] || ($mydir==$to && $this->total_folders==0)) && is_dir($mydir)) ){
			@chmod($mydir,0777);
			$this->total_folders ++;
			echo "&nbsp;&nbsp;&nbsp;&nbsp;<a href='$mydir' target='_blank'><b>目录:</b> $mydir</a><br>";
		}
	}
...
<?php
elseif($_POST['myaction'] == "dodelete"):
	$dfile = $_POST['dfile']; 
	echo "正在删除文件...<br><br>";
	if(is_array($dfile)){
		for($i = count($dfile) - 1; $i >= 0; $i--){
			if(is_file($dfile[$i])){
				if(@unlink($dfile[$i])) echo "已删除文件: $dfile[$i]<br>";
				else echo "删除文件失败: $dfile[$i]<br>";
			}	
		}
	}
